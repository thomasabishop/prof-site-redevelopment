...
We can clearly see that as n grows, the runtime gets steeper and more pronounced. This trend will increase
so long as <strong>n</strong> grows.
...
<ul>
  <li>
    We are assigning <code>0</code> to <code>total</code> once (<strong>O(1)</strong>)
  </li>
  <li>
    We are increasing <code>i</code> by <code>1</code> on each iteration (<strong>O(n)</strong>)
  </li>
  <li>
    We are updating the value <code>total</code> to the sum of
    <code>total</code> plus <code>array[i]</code> on each iteration (<strong>O(n)</strong>
    ).
  </li>
</ul>
